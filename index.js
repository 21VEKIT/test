...
setInterval(() => {
    ...
    model_buff.fetchPercentMain().then(percent_1 => {
        model_buff.fetchPercent2().then(percent_2 => {
            axios({
                url: `https://buff.163.com/api/market/goods?game=csgo&page_num=1&use_suggestion=0&min_price=${percent_1.min}&max_price=${percent_1.max}`,
                method: 'GET',
                headers: {
                    cookie: `session=${percent_1.session}`
                }
            }).then(response_goods => {
                model_buff.fetch_error_goods().then(error_goods => {

                    if(response_goods.data.code === 'User Frozen')
                        return false
